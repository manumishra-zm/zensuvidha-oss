...
| **Answers without the model** | The questions people repeat word-for-word are answered straight from the pack: **0.45 ms against 3219 ms**, and ~20 s in Telugu where the same reply inflates 6.2×. It cannot invent, because the sentence *is* the fact. Declines unless the match is unmistakable — measured English 0.63–1.00 against off-topic 0.15–0.41 — and never fires mid-booking. |
| **Sounds like a person** | The repair line says *why* it could not hear — the room's floor-to-voice gap is measured every turn, so "there's a lot of background noise, could you move somewhere quieter?" replaces a flat "say that again". A finished phone number or a bare "haan" closes the turn at 400 ms instead of 1200. And a long turn gets a quiet "mm-hm" at a natural pause, so the agent is not silent then abruptly speaking. |
| **Echo** | `echo.py` — all echo handling used to come from `getUserMedia`, so on a phone line the agent would hear itself, treat it as barge-in, and interrupt itself in a loop. Normalised cross-correlation against what we played: our own voice scores 0.99–1.00 at any delay or attenuation, an unrelated speaker ≤0.27. Suppression, not adaptive cancellation — it refuses a frame rather than subtracting from it, because refusing can only lose echo while subtracting badly corrupts the caller. |
| **Speculative reply** | The answer is generated while the caller may still be pausing, and adopted only if the committed transcript is *identical*. Strictly read-only on the session — a guess never mutates state, which this codebase learned the hard way. |
| **Transport** | `transport.py` — a four-method seam so the audio front-end is not browser-coupled, with μ-law/resampling and the endpointer a carrier does not give you. Optional Pipecat adapter for Exotel/Plivo, transport only. |
